...
extractStringPropertyFromObject(JSContext *cx, JS::HandleObject object, std::string_view property) {
  ...
  auto chars = saru::encode(cx, value);
  if (!chars) {
    return JS::Result<std::optional<std::string>>(JS::Error());
  }
    ...
    if (has_key_ops) {
      ...
      if (length != 0) {
        ...
        for (uint32_t i = 0; i < length; ++i) {
          ...
          auto op_chars = saru::encode(cx, op_val);
          if (!op_chars) {
            return nullptr;
          }
    ...
    if (has_oth) {
      ...
      if (length != 0) {
        ...
        for (uint32_t i = 0; i < length; ++i) {
          ...
          auto r_chars = saru::encode(cx, info_val);
          if (!r_chars) {
            JS_ReportErrorASCII(cx, "Failed to read the 'oth' property from 'JsonWebKey': The "
                                    "provided value is not of type 'RsaOtherPrimesInfo'");
            return nullptr;
          ...
          auto d_chars = saru::encode(cx, info_val);
          if (!d_chars) {
            JS_ReportErrorASCII(cx, "Failed to read the 'oth' property from 'JsonWebKey': The "
                                    "provided value is not of type 'RsaOtherPrimesInfo'");
            return nullptr;
          ...
          auto t_chars = saru::encode(cx, info_val);
          if (!t_chars) {
            JS_ReportErrorASCII(cx, "Failed to read the 'oth' property from 'JsonWebKey': The "
                                    "provided value is not of type 'RsaOtherPrimesInfo'");
            return nullptr;
